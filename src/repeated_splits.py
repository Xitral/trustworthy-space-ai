...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Joblib backend. 'loky' uses processes. 'threading' avoids copying "
            "DataFrames on Windows."
        ),
    )
    parser.add_argument(
        "--skip-uncertainty",
...
def bootstrap_ensemble_predict(
    ...
    """Fit bootstrap models and return predictive mean and standard deviation."""
...
def run_horizon_job(
    ...
    """Run one split/horizon evaluation job."""
    metrics_rows = []
    escalation_rows = []
    ...
    with threadpool_limits(limits=1):
        ...
        gb_model = build_gradient_boosting_model(model_seed, max_iter)
        ...
        ensemble_mean_prob, ensemble_std_prob = bootstrap_ensemble_predict(
            x_train=x_train,
            y_train=y_train,
            x_test=x_test,
...
def build_horizon_jobs(df: pd.DataFrame, split_seeds: list[int]) -> list[dict]:
    """Build deterministic split/horizon jobs for parallel execution."""
    jobs = []
